...
Для **запуска только модуля поиска неоднозначности** (без логирования в LaTeX и возможности использовать другие команды) необходимо вызвать следующий скрипт (по умолчанию регулярное выражение читается из файла `input.txt`)
```
str=`cat input.txt`
