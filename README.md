...
Для **запуска только модуля поиска неоднозначности** (без логирования в LaTeX и возможности использовать другие команды) необходимо вызвать следующий скрипт (по умолчанию регулярное выражение читается из файла `input.txt` - без внешних фигурных скобок)
```
str=`cat input.txt`
str2="X = Ambiguity.Thompson {"$str"}"
echo $str1 > test1.txt
time ( timeout 30 ./InterpreterApp test1.txt -n > result.txt; ) 2>time.txt
```
